export function parseRawBiometrics(text) {
  ...
  const LATE_CUTOFF_MIN  = 15    // Late if shift in is after 7:15 AM
  const MIN_HOURS        = 8.5   // Undertime if total hours < this
  ...
  for (const entry of Object.values(punchMap)) {
    ...
    const lunchOut = (entry.ins.length >= 2 && entry.outs.length >= 1)
      ? entry.outs[0]
      : null

    const lunchIn = (lunchOut !== null && entry.ins.length >= 2)
      ? entry.ins[1]
      : null
    ...
    if (lunchOut !== null) {
      if (entry.outs.length >= 2) {
        shiftOut = entry.outs[entry.outs.length - 1]
      }
    } else {
    ...
    // ── Status ───────────────────────────────────────────────
    // No shift in at all → Absent
    // Has shift in but no shift out → Incomplete (showed up, missing tap)
    // Has both → check late + undertime
    let status = 'Absent'
