...
export default function VisitDeleteModal({ open, visit, onConfirm, onClose }) {
  const name = visit?.fullName || visit?.employee || "this employee"
    ...
    <Dialog open={open} onOpenChange={val => { if (!val) onClose() }}>
      <DialogContent className="sm:max-w-sm bg-white outline-none focus:outline-none ring-0 focus:ring-0 border-0">
        ...
        <p className="text-sm text-gray-500 text-center px-2 pb-2">
          Are you sure you want to remove the visit record for{" "}
