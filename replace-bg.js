...
function walk(dir) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // Replace plain bg-white, bg-gray-100 etc that should use theme shades
      // We'll replace common surface colors with theme-50 for light and dark:bg-gray-900 for dark
      const surfaceMap = {
      ...
      for (const [oldCls, newCls] of Object.entries(surfaceMap)) {
        ...
        content = content.replace(regex, `${newCls} dark:${newCls.replace('var(--theme-', 'var(--theme-').replace(')]', ')]')}`);
      }
      if (content !== original) {
